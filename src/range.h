// Copyright © 2013, Mark Harris
// https://github.com/harrism/cpp11-range
#ifndef RANGE_H_
#define RANGE_H_
...
struct range_proxy {
  struct iter : detail::range_iter_base<T> {
    explicit iter(T current) : detail::range_iter_base<T>(current) {}
  };
...
struct infinite_range_proxy {
  struct iter : detail::range_iter_base<T> {
    explicit iter(T current = T()) : detail::range_iter_base<T>(current) {}
  ...
  struct step_range_proxy {
    struct iter : detail::range_iter_base<T> {
      explicit iter(T current = T(), T step = T())
          : detail::range_iter_base<T>(current), step(step) {}
  ...
  explicit infinite_range_proxy(T begin) : begin_(begin) {}
